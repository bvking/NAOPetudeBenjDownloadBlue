  ...
  */
   ...
   /*
    if (phasePatternFollow[i]<0){
     dataToControlStepMotorisedPosition[i]= int (map (phasePatternFollow[i], 0, -TWO_PI, numberOfStep, 0)); 
    net.oldPhase[i]= net.phase[i];
  //  net.phase[i]= map (dataToControlStepMotorisedPosition[i], numberOfStep, 0, 0, -TWO_PI);
       }
    ...
    dataToControlStepMotorisedPosition[i]= (int) map (phasePatternFollow[i], 0, TWO_PI, 0, numberOfStep); 
    net.oldPhase[i]=net.phase[i];
  //  net.phase[i]= map (dataToControlStepMotorisedPosition[i], 0, numberOfStep, 0, TWO_PI);
    }
   */
...
/*
   if (signal[2]<0){
     dataToControlStepMotorisedPosition[0]= int (map (signal[2], 0, -TWO_PI, numberOfStep, 0));  
     signal[2]= map (dataToControlStepMotorisedPosition[0], numberOfStep, 0, 0, -TWO_PI);
   //   newPosF[i]= phaseMapped[i];
       }

   else {
    dataToControlStepMotorisedPosition[0]= (int) map (signal[2], 0, TWO_PI, 0, numberOfStep); 
    signal[2]= map (dataToControlStepMotorisedPosition[0], 0, numberOfStep, 0, TWO_PI); 
  }
*/
   for (int i = 0; i <  networkSize-0; i+=1) {
