...
void samplingMovement(float timeSec) {
  ...
  print ("             Timer2  ");
  print ( Timer2);
  print ("             frameSampling160  ");
  print ( frameSampling%(num*timeSec)); // each x frameSampling record new datas
  ...
  if ( (frameSampling >=num*timeSec+1 && frameSampling <=num*timeSec+1)) { //|| TimeMiddleElapsed >=1970 && TimeMiddleElapsed <=2030 ||
    ...
    print (frameSampling);
    print ("  TimeMiddleElapsed ");
    println ( TimeMiddleElapsed);
  }
  ...
  if (frameSampling>=num*timeSec+1  ) // begin to replay for 2 sec
  {
    ...
    print (i);
    print ("  ");
    print (ry[i]);   // frameSampling%160=1 is the first point and  frameSampling%160=0 is the last point recorded
    print (" my ");
    print (i);
    print ("  ");
    println (my[i]);
  ...
  if (frameSampling%(2*num*timeSec)<=0  ) //
  {
    ...
    print (" LastTimeElapsed");
    println (LastTimeElapsed);
    //   Timer=0;
    //   Timer2=0;
    //   frameSampling=80;
...
void followDistribueAddphasePattern() {
  ...
  if (formerFormerKey == '#'  || formerKeyMetro == '*'  || formerKeyMetro == '*' ) { // || formerKeyMetro == '*'
    ...
    if (formerFormerKey != '#' ) {
      ...
      for (int i = 0; i < networkSize-0; i+=1) {
        print ("  BEF phaseMapped[i]  ");
        println ( phaseMapped[i]  );
...
void followDirectLfo() {
  ...
  if (formerKey != '#' ) { //  play this command once when you tap on keyboard expect #
    if (doA==true ) { // offset with pendularPattern  || doQ==true
      ...
      print ("  case a phaseFollowLFO  ");
      print ( oscillatorChange  );
      print ( " ");
      print ( phaseFollowLFO[oscillatorChange]  ) ;
      println (   );
      //   LFO[i]=0;
...
void addSignal() {
  ...
  if (formerFormerKey == '#') { //  && doA==true
    print ("  normal " + frameCount + " lfoPhase[1] " + lfoPhase[1] + " lfoPhase[2] " + lfoPhase[2]);
    println (   );
  ...
  if (doQ==true ) {
    ...
    print ("  LFOoscillatorChange  ");
    print (oscillatorChange);
    print ("   ") ;
    print (LFO[oscillatorChange]  );
  ...
  print ("  LFO+LFOoscillatorChange  ");
  print (oscillatorChange);
  print ("   ") ;
  println (LFO[oscillatorChange]  );
