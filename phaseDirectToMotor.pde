 ...
 {   
    ...
    {
     for (int i = 0; i < networkSize; i++) {
         lastActualPosition[i]= lastActualPosition[i] + (int)  map ( (networkSize-1- oscillatorBlocked-i)*TWO_PI/networkSize*1/8, 0, TWO_PI, 0, numberOfStep);   
          }
    }
    ...
    {
    for (int i = 0; i < networkSize; i++) {
        lastActualPosition[i]= lastActualPosition[i] - (int)  map ( TWO_PI*1/8/(networkSize)*(networkSize-1-oscillatorBlocked-i), 0, TWO_PI, 0, numberOfStep);
         }
    }
